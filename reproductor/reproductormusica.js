...
function playNextSong() {
    ...
    // Si el índice es igual a la longitud del array, volver al principio
...
playlist.songs.forEach((song,index) => {
    ...
    playBtn.addEventListener("click", async function () {
        reproducirCancion(playlist.songs[index]);
    });
  // 2. BOTÓN DE FAVORITO (CORAZÓN) y su comportamiento en MIS FAVORITOS
    ...
    favselBtn.addEventListener("click", function () {
        ...
        if (!isSongAdded) {
            ...
            //A2. prev, atras, pause, mute, vol, play
            const stopBtn = document.getElementById('stop-btn')
            stopBtn.addEventListener('click', function () {
                currentAudio.pause(); // Pausar la reproducción de la canción actual
                // No reiniciar currentSongIndex aquí
            });
    ...
    addBtn.addEventListener("click", function () {
        ...
        if (!isSongAddedToFavorites) {
            ...
            //A3. prev, atras, pause, mute, vol, play
            const stopBtn = document.getElementById('stop-btn')
            stopBtn.addEventListener('click', function () {
                currentAudio.pause(); // Pausar la reproducción de la canción actual
                // No reiniciar currentSongIndex aquí
...
boton.addEventListener('click', function () {
    ...
    const cancionesFiltradas = playlist.songs.filter(song => {
        const nombreMatch = song.nombre.toLowerCase().includes(valor);
        const generoMatch = song.genero.toLowerCase().includes(valor);
        const artistaMatch = song.artista.toLowerCase().includes(valor);
        return nombreMatch || generoMatch || artistaMatch;
    });
    ...
    cancionesFiltradas.forEach(song => {
        ...
        // Lista mis favoritos en Buscador
        const myPlaylistContainer = document.querySelector('#myplaylist');
        ...
        favselBtn.addEventListener("click", function () {
            ...
            if (!isSongAdded) {
                ...
                // Agregar el botón de reproducción al elemento clonado
                const playBtn = createPlayBtn(song);
                clonedSong.appendChild(playBtn);
                playBtn.addEventListener("click", async function () {
                    reproducirCancion(song);
                ...
                //A4. prev, atras, pause, mute, vol, play
                const stopBtn = document.getElementById('stop-btn')
                stopBtn.addEventListener('click', function () {
                    currentAudio.pause(); // Pausar la reproducción de la canción actual
                    // No reiniciar currentSongIndex aquí
        ...
        // Agregar el botón de reproducción al elemento <li>
        songElement.appendChild(playBtn);
        ...
        // Agregar desde el buscador (SUMA)
        const addBtn = document.createElement("button");
        addBtn.classList.add("fa-solid", "fa-plus");
        addBtn.addEventListener("click", function () {
